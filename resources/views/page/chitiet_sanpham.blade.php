		...
		<div class="modal-content">
			...
			<form action="{{ route('add-rating') }}" method="POST">
				...
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">Rate {{ $sanpham->name }}</h5>
...
<span class="flash-sale">{{number_format($sanpham->promotion_price)}} Đồng</span>
							    ...
							    <div class="rating">
								@for ($i = 1; $i <= 5; $i++) @if(isset($averageRating)) @if ($i <=$averageRating) 
								<i class="fa fa-star checked"></i>
									...
									@if ($ratingCount > 0)
									<p>Số lượng đánh giá: {{ $ratingCount }}</p>
									@endif
									<p>{{ $averageRating }}</p>
									@endif

									<a href="{{ route('products.show', ['id' => $sanpham->id]) }}">Xem chi tiết</a>
							</div>
